    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            LittleLemonTheme {
                ...
                print(orderMenuItems)

                // add menuItems variable here
                var menuItems = if (orderMenuItems.value == true) {
                ...
                Column(
                    ...
                    Button(onClick = {orderMenuItems.value = true}, modifier = Modifier.align(CenterHorizontally)) {
                        Text(text = "Tap to Order By Name")
                    }
                    // add searchPhrase variable here
                    var searchPhrase by remember { mutableStateOf("") }
                    // Add OutlinedTextField
                    OutlinedTextField(value = searchPhrase, onValueChange = {searchPhrase = it}, modifier = Modifier
                        .fillMaxWidth()
                        .padding(start = 50.dp, end = 50.dp), label = {Text("Search")})
                    // add is not empty check here
                    if (searchPhrase.isNotEmpty()) { MenuItemsList(items = menuItems.filter {
                        it.title.contains(
                            searchPhrase,
                            ignoreCase = true
                        )
